...
const generateWaveData = (offset) => {
  const getDemandValue = (hour) => {
    if (hour <= 3) return 15000 - (hour * 700); // Sharp decline
    if (hour <= 6) return 13000 - ((hour - 3) * 600); // Continued decline
    if (hour <= 9) return 11000 + Math.sin(hour) * 100; // Stable around 11k
    if (hour <= 12) return 11000 + ((hour - 9) * 400); // Slight rise
    if (hour <= 15) return 12200 - ((hour - 12) * 200); // Small decline
    if (hour <= 18) return 11500 + Math.sin(hour) * 200; // Stable
    if (hour <= 21) return 11500 + ((hour - 18) * 1000); // Sharp rise
    return 14500 + Math.sin(hour) * 100; // Stable high
  };

  return Array(24).fill().map((_, i) => ({
    time: i,
    // Wind: starts around 600, gradually increases to ~800-1000 with fluctuations
    windValue: 600 + Math.sin((i + offset) / 8) * 100 + i * 10,
    windForecast: 590 + Math.sin((i + offset) / 8) * 100 + i * 10,
    
    // Solar: zero at night, peak around 300-400 during day
    solarValue: i >= 6 && i <= 18 
    ...
    // Demand: More accurate pattern matching the image
...
export default function Home() {
  ...
  const handleDemoClick = () => {
    window.open('https://energypredictor.streamlit.app/', '_blank');
  };
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data}>
                  ...
                  <XAxis 
                    ...
                    axisLine={false}
                    tickLine={false}
                    tick={false} // Removes the numbers
                  />
                  <YAxis 
                    stroke="#9CA3AF"
                    axisLine={false}
